...
\subsection{Module and Module Dependency}

A modular application may contain more than one module which is consisted of one or more classes. 
A module can be defined as $M=<C, R, L>$. $C=\{c_1, c_2, c_3\cdots\}$ is the set of classes in that module. 
...
\caption{main source codes of two session beans}
...
In module Compute.jar, $R = \{Validator.jar\}$, because class \emph{ComputeBean} calls the method in class \emph{ValidatorBean} and these two classes are in different modules. 
In module Validator.jar, $R = \emptyset$, which means Validator.jar doesn’t depends on any modules. 
...
Module class loaders will load classes from each module separately.
...
For the division application in Fig.\ref{fig:example}, when two modules are deployed, the classes in the modules are loaded by module class loaders. 
...
\caption{dependency reconstruction in appliaction servers}
...
The dependent class loaders will be set in the CDR Class Loader of each module.
...
We regard the system class loader as a dependent class loader of every module class loader to guarantee the safe loading of system classes. 
...
In CDR Class Loader, reconstruction process is similar to the process of creating dependency graph. 
The graph and CDR Class Loaders are updated by the new version of profiles before the loading of classes. 
Then all of modules which depend on updating modules also require to be updated even though they have no new versions. 
...
Editing profiles of deployment nodes is a quite messy work when modular applications are composed of hundreds of modules.

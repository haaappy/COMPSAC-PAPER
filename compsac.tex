...
Fig. \ref{fig:jboss_class_loader} shows an illustrative architecture of class loading in JBoss and other application servers adopt the similar design. As for JBoss,  \emph{ServerLoader} is responsible to load JBoss server itself. For applications of different types, including EARs, JARs, WARs, etc., seen by the deployment scanner, the server applies various loaders, including \emph{EJB ENCLoader}, \emph{Web ENCLoader}, to load classes respectively. In advance, each class loader delegate the \emph{UnifiedClassLoader}  to mange the classes loaded in a repository. Figure \ref{fig:example} shows two EJBs packed in JARs. Each EJB contains only one class here for simplicity. Class ComputeBean in Computer.jar is a session bean for dividing calculation. Class ValidatorBean is another session bean that verifies whether the a number is zero or not. If these two modules are deployed in JBoss, two \emph{EJB ENCLoader} instances are responsible for loading the classes and they delegate two \emph{UnifiedClassLoader} instances to load classes. So each \emph{UnifiedClassLoader} instance serves the corresponding module as its module class loader.
...
The modules in the above example actually have an internal dependency between them as ComputeBean uses ValidatorBean's \emph{iszero} method to verify whether the divisor is zero or not by referring the latter class. Such a reference takes effect because while the classes are loaded into the JVM, the information of the classes are resolved by their class loaders and stored in their own class structure tables in the virtual machine. As for the reference against $ValidatorBean$, the \emph{field\_info} table of $ComputeBean$ stores the address of the class structure of $ValidatorBean$, as shown in Fig.\ref{fig:example_jvm}.  In short, we can find the field name \emph{validator} and field type \emph{ValidatorBean} from the index in the \emph{field\_info} of class structure \emph{ComputeBean}. 
...
At the first time of deployment, the dependency can be captured by the server as ....  However, if the depended module is hot deployed again later, for example, if the validator class raises its precision by ... and the new packed JAR file are dropped into the server, calling ComputeBean then will throw an exception. The reason is that according to the JVM specification\cite{jvm_specification}, a class loader cannot load a class twice, which means we cannot use the original Class Loader  to load the new version of Class \emph{ValidatorBean}. So the server will simply destroy the existing class loader for VJAR and create a new one for the lately deployed one while the CJAR remains untouched. The class structure of Class \emph{ValidatorBean} in JVM is  is recreated and maintained in a new place in memory. In this case, the original reference becomes invalidate. Dependency between the two modules are broken and it consequently cause a failure.
...
Dependencies among modules can be explicitly stated by the application developers. ...

To provide such a explicit model for dependencies is tedious. Actually some component technology has mechanism to specify the inter-relationship between components. For example, EJB provides resource injection mechanism as component assembling. If the module has description for this kind of dependency, it can be used as find out ...

However, the injections are not complete. For example, some module can include SPI (service provider interfaces) and other modules are the implementation for those interfaces. In this case, they are still dependent but no component level injections are appropriate for this. 

For this case, application server can correct order to ... (howto ) But it cannot preciously ...

So, reconstruction in application server is not feasible.

...
However, most applications are modular applications which are consisted of more than one module containing one or more classes.
...
The current application servers have hot deployment capabilities using module class loader architecture.
If a single module as an application hot deploys, the module class loader will also update with it and application will be running regularly.
...
For the division application, when two modules are deployed, the classes in the modules are loaded by module class loaders shown in Fig.\ref{fig:example_jvm}. 
...
\begin{algorithm}[h]
\caption{function loadClass of Circle Class Loader}
...
We implement Circle Class Loader with static dynamic management.
Algorithm \ref{alg:Circle_Class_Loader} shows Circle Class Loader how to load classes. 
...
In Circle Class Loader, reconstruction process is similar to the process of creating dependency graph. 
The graph and Circle Class Loaders are updated by the new version of profiles before the loading of classes. 
The dependencies are managed in the Circle Class Loaders based on the static profiles of modules.
